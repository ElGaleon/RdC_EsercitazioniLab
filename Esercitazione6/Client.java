...
class Client{
  ...
  // Avvio del Client RMI
	public static void main(String[] args) {
		...
		String serviceName = "JavaRMI";
		BufferedReader stdIn = new BufferedReader(new InputStreamReader(System.in));

		// Controllo dei parametri della riga di comando
		if (args.length != 1 && args.length != 2) {
			System.out.println("Sintassi: Client RegistryHost [registryPort], registryPort intero");
			System.exit(1);
		}
		...
		try {
			...
			while ((service = stdIn.readLine()) != null) {
				...
				if (service.equals("C")) { // l'utente richiede il servizio conta_righe

					printf("Inserisci nome file: ");
					fileName = stdIn.readLine();


					printf("Inserisci numero minimo occorrenze");
	...
	/*				boolean ok = false;
				...
				else if (service.equals("E")) { // utente chiede servizio elimina_righe

					printf("Inserisci nome file: ");
					fileName = stdIn.readLine();


					printf("Inserisci numero linea da eliminare");
