				...
				if(service.equals("R")){
					...
					while (ok != true){
						...
						if(!sess.equals("S1") && !sess.equals("S2") && !sess.equals("S3")
							...
							System.out.print("Sessione (S1 - S12)? ");
							continue;
						} else ok = true; 	//Sessione inserita valida
					}
					
					System.out.print("Speaker? ");
					String speak = stdIn.readLine(); //qualsiasi nome accettato, nessun check
					
					if(serverRMI.registrazione(g, sess, speak) == 0) System.out.println(
							"Registrazione di " + speak + " effettuata per giornata " 
								  + g + " sessione " + sess);
					else System.out.println("Sessione piena: giornata" + g + " sessione "
							+ sess);
				} // R=Registrazione
